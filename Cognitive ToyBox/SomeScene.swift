...
class SomeScene: SKScene, ConfigurableScene {
...
//      self.runAction(SoundSourceHelper.soundTouch())
  ...
  func nextSession () {
    ...
    self.objectList = self.gameController.getListWithSameMaterial().filter() { [unowned self] (object:CognitiveToyBoxObject) -> Bool in
    ...
    // refresh main
    self.objectName = self.gameController.getMainObj().material!
    self.descriptionLabel.text = "Some \(objectName)"
    self.questionLabel.text = "Some \(objectName)"
    
    
    
    self.wrongLabel.text = "That's not some \(objectName)!"
