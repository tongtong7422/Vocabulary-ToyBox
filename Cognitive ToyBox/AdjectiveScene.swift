...
class AdjectiveScene: SKScene, ConfigurableScene {
  ...
  func nextSession () {
    ...
    // refresh main
    var mainObject = self.gameController.getMainObj()
    self.objectName = mainObject.name
    var adj:String
    
    switch self.gameController.task {
    case .Material:
      objectList = CognitiveToyBoxObjectSearchHelper.getSameNameObjectListOnMaterial(name: mainObject.name, exceptId: mainObject.id, material: mainObject.material!, color: mainObject.color!)
      adj = mainObject.material!
    case .Color:
      objectList = CognitiveToyBoxObjectSearchHelper.getSameNameObjectListOnColor(name: mainObject.name, exceptId: mainObject.id, material: mainObject.material!, color: mainObject.color!)
      adj = mainObject.color!
    case .Length:
      adj = "long"
    default:
      adj = ""
    ...
    self.descriptionLabel.text = "This is a special kind of \(objectName). It is a \(adj) \(objectName)!"
    
    self.questionLabel.text = "What do you think \(mainObject.pluralName) usually look like?"
    
    
    
    self.wrongLabel.text = "That's not a usual \(objectName)!"
  ...
  func displayMain () {
    ...
    self.mainObjectNode.runAction(ActionHelper.presentObject()) {
      ...
      self.mainObjectNode.runAction(SKAction.sequence([ActionHelper.waitForMain(), ActionHelper.waitForMain()])) {
        [unowned self] () -> () in
        self.mainTouched()
      }
    }
  ...
  func initOptions () {
    ...
    if self.gameController.task == .Length {
      self.correctNode.xScale /= 2
    }
